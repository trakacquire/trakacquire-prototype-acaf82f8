...
export default function BroadcastsPage() {
  ...
  const columns: ColumnDef<BRow>[] = [
    ...
    { header: 'Enviados', accessorKey: 'sent', className: 'text-right', cell: (b) => (
      <span className="font-mono text-13 text-eggshell tabular-nums">{b.sent.toLocaleString('pt-BR')}</span>
    )},
        ...
        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <button
                onClick={() => openEvidence(buildEvidence({
                  ...
                  value: <span className="font-mono text-verified">{consentAllowed.toLocaleString('pt-BR')} permitidos · {consentBlocked} bloqueados</span>,
              ...
              >
                ...
                <div className="text-13 text-eggshell">Consentimento verificado <span className="font-mono tabular-nums">{consentAllowed.toLocaleString('pt-BR')}</span></div>
              ...
              <button
                onClick={() => openEvidence(buildEvidence({
                  ...
                  value: <span className="font-mono text-verified">re-check antes do POST</span>,
          ...
          {showComposer && (
            <div className="bg-graphite border border-line rounded-xl p-5 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <label className="flex flex-col gap-1">
                  ...
                  <select className="bg-zinc border border-line rounded-md px-3 py-2 text-13 text-eggshell">
                    {SEGMENTS.map(s => <option key={s.id}>{s.name} ({s.count.toLocaleString('pt-BR')})</option>)}
                  </select>
                </label>
                <label className="flex flex-col gap-1">
                  ...
                  <input value={window} onChange={e => setWindow(e.target.value)} className="bg-zinc border border-line rounded-md px-3 py-2 text-13 text-eggshell font-mono" />
                </label>
                <label className="flex flex-col gap-1">
              ...
              <div className="flex justify-end gap-2">
                ...
                <button onClick={() => { toast('Broadcast agendado — passa por Policy Engine na fila e antes do envio.'); setShowComposer(false); }} className="bg-eggshell text-ink px-4 py-2 rounded-md text-13 font-medium">Agendar</button>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <button onClick={() => openEvidence(buildEvidence({
              ...
              <div className="text-24 font-bold text-eggshell font-mono tabular-nums mt-2">{SEGMENTS.length}</div>
            </button>
          </div>
