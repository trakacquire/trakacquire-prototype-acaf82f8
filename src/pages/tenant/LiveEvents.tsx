  ...
  return (
    <AppShell breadcrumb={[{ label: 'Observe', href: '/live' }, { label: 'Eventos ao Vivo' }]}>
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* ── Stream com gate de cenário (8 estados) ──────────────────── */}
        <ScenarioStateGate
          emptyTitle="Sem sinais no stream"
          emptyDescription="Nenhum evento chegou nos últimos 5 minutos."
                        ...
                        >
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2.5">
                              ...
                              <div className="min-w-0">
                                ...
                                <div className="text-11 font-mono text-stone truncate tabular-nums">{evt.id}</div>
                              </div>
                            </div>
                          </td>
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3">
                            <Link href={`/players/${evt.person_id}`} className="block group">
                              ...
                              <div className="text-11 font-mono text-stone tabular-nums">{evt.person_id}</div>
                            </Link>
                          </td>
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3 text-right">
                            <MetricValue
                              value={evt.value != null ? `R$ ${evt.value.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}` : '—'}
                              size="sm"
                              tone={evt.status === 'Reconciled' ? 'verified' : evt.status === 'Divergent' ? 'warning' : 'default'}
                              onOpenEvidence={() => openEvidenceFor(evt)}
                            />
                          </td>
                          <td className="px-4 py-3 text-right">
                          ...
                          <td className="px-4 py-3 text-right">
                            <span className="text-12 text-stone whitespace-nowrap tabular-nums">{relativeTime(evt.timestamp)}</span>
                          </td>
                        </motion.tr>
                      );
                    })}
